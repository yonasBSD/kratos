...
func TestHandler(t *testing.T) {
	...
	t.Run("suite=create and update", func(t *testing.T) {
		...
		t.Run("case=should update an identity with lookup_secret credentials", func(t *testing.T) {
			...
			actual, err := reg.PrivilegedIdentityPool().GetIdentityConfidential(t.Context(), uuid.FromStringOrNil(id))
			require.NoError(t, err)
			...
			send(t, adminTS, "PUT", "/identities/"+id, http.StatusOK, &identity.UpdateIdentityBody{
		...
		t.Run("case=should update an identity with totp credentials", func(t *testing.T) {
			...
			assert.Equal(t, "totp://example.com?secret=JBSWY3DPEHPK3PXP&issuer=ORY", totpConfig.String(), "TOTP secret should be stored correctly")
			...
			send(t, adminTS, "PUT", "/identities/"+id, http.StatusOK, &identity.UpdateIdentityBody{
				...
				Credentials: &identity.IdentityWithCredentials{
					TOTP: &identity.AdminIdentityImportCredentialsTOTP{
						Config: identity.AdminIdentityImportCredentialsTOTPConfig{
							TOTPURL: "totp://example.com?secret=NBSWY3DPEHPK3PXQ&issuer=ORY", // Example TOTP URL
						},
					},
				},
			})
		...
		t.Run("case=should update an identity with oidc credentials", func(t *testing.T) {
			...
			var config identity.CredentialsOIDC
		...
		t.Run("case=should update an identity with saml credentials", func(t *testing.T) {
			...
			var config identity.AdminIdentityImportCredentialsSAMLConfig
...
func getCodeValues(codes []identity.RecoveryCode) []string {
	var values []string
